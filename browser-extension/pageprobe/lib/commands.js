...
function excludeFromExtra(field, excluded) {
  ...
  if (['submit', 'button', 'reset', 'image'].includes(field.type)) {
    // Control inputs are part of the submitted request only when named and
    // valued — e.g. DVWA gates auth on `isset($_GET['Login'])`. Unnamed or
    // empty controls are never submitted by a browser, so skip those.
    if (!field.name || field.value === '') return true;
  }
  return field.value === '';
}
